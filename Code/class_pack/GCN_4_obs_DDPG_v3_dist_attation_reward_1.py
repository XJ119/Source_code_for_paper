...
class GCN_Critic_DDPG(nn.Module):
    def __init__(self, a_dim = 2, s_input_dim=4,agent_num = 7, obs_num=6):
        ...
        self.obs_num = obs_num
        self.cnn_state_1 = GCN_layer_2(s_input_dim,agent_num*2)
        self.cnn_state_atten = GCN_layer_2(s_input_dim,agent_num*2)
        self.cnn_state_atten_obs = GCN_layer_2(s_input_dim,agent_num*2)
        self.cnn_state_obs_1 = GCN_layer_2(s_input_dim,agent_num*2)
        self.mpl_state_1 = nn.Linear(s_input_dim,agent_num*2)
        self.cnn_action_1 = GCN_layer_2(a_dim,agent_num*2)
        ...
        self.out2 = nn.Linear(agent_num*8*agent_num,agent_num*2*agent_num) #6为全图节点个数-1。
        #self.out1.weight.data.normal_(0, 0.1)  # initialization
        self.out3 = nn.Linear(agent_num*2*agent_num,1) #6子图节点个数，1行为数。
        #self.out2.weight.data.normal_(0, 0.1)  # initialization
        #self.out_action = nn.Linear(50,1)
    def graph_softmax(self, adj ,dist_Q, dist_K):
        ...
        #temppp= dist_Q.view(dist_Q.size(0),dist_Q.size(1),-1)
    ...
    def forward(self,adjacency,adjacency_self,state_features,adjacency_obs,state_features_obs, action_features,k):
        #temp_1 = adjacency*state_features[:,0:int(self.agent_num):]
        attention_temp_k = torch.zeros(adjacency.shape[0],1,adjacency.shape[3]).to(device)
        attention_temp_a_k = torch.zeros(adjacency.shape[0],1,adjacency.shape[3]).to(device)
        attention_temp_k[:,:,0] = 1.0
        attention_temp_k = attention_temp_k+adjacency[:,k:k+1,0,:]
        attention_temp_a_k [attention_temp_k>0.0] =1.0 
        attention_temp_t_k = attention_temp_a_k.permute(0,2,1)
        attention_temp_obs = adjacency_obs[:,k:k+1].permute(0,2,1)
        attention_temp_obs [attention_temp_obs>0.0] =1.0 
        ...
        #注意力权重计算
        h_s_i_atten_Q = F.relu(self.cnn_state_atten(state_features[:,:,0:int(self.agent_num)]))
        temp_qk_a = self.graph_softmax(attention_temp_t_k,h_s_i_atten_Q[:,k],h_s_i_atten_Q[:,k,0:1])

        h_s_i_obs_atten_Q = F.relu(self.cnn_state_atten_obs(state_features_obs))
        temp_qk_a_obs = self.graph_softmax(attention_temp_obs,h_s_i_obs_atten_Q[:,k],h_s_i_atten_Q[:,k,0:1])
     ...
     #   attention_temp_self = adjacency_self[:,k:k+1,0,:]
        ...
        #attention_temp_k = adjacency_self[:,k:k+1,0,:]
       # attention_temp_a_k [attention_temp_self>0.0] =1.0 
       # attention_temp_t_k = attention_temp_a_k.permute(0,2,1)

        attention_data_i = (torch.matmul(temp_qk_a,h_s_i)).squeeze(2)    
        attention_data_1 = F.relu(self.attantion_layer(attention_data_i))


        
        attention_data_i_obs = (torch.matmul(temp_qk_a_obs,h_s_i_obs)).squeeze(2)
        ...
        h_attention_self = attention_temp_t_k*self_dist_attention_q
