...
class GCN_Critical(nn.Module):
    def __init__(self, a_dim = 2, s_input_dim=4,agent_num = 7):
    ...
    def forward(self,adjacency,state_features,adjacency_obs,state_features_obs, action_features):
        ...
        h_s_i = F.relu(self.cnn_state_1(state_features[:,0,0:int(self.agent_num)]))
        h_s_i_obs = F.relu(self.cnn_state_obs_1(state_features_obs[:,0,:]))
 
        h_s_e_1 = F.relu(self.mpl_state_1(state_features[:,0,int(self.agent_num):int(self.agent_num)+1])).squeeze(2)
...
class GCN_Actor(nn.Module):
    def __init__(self, input_dim=4,sub_agent_num=4):
        super(GCN_Actor, self).__init__()
        self.agent_num = sub_agent_num
        self.agent_num_obs = sub_agent_num
