...
class GCN_Actor(nn.Module):
    def __init__(self, input_dim=4,sub_agent_num=4, obs_num=6):
        ...
        self.atten_Q = GCN_layer_2(input_dim,sub_agent_num*2)
        ...
        self.atten_obs_Q = GCN_layer_2(input_dim,sub_agent_num*2)
    ...
    def graph_softmax(self, adj ,dist_Q, dist_K):
        ...
        return dist_weight.permute(0,2,1)
    ...
    def forward(self,attention_weight,state_features,attention_weight_obs,state_features_obs):
        ...
        h_aten_Q = F.relu(self.atten_Q(state_features[:,0:int(self.agent_num):]))
        temp_qk_a = self.graph_softmax(attention_temp.permute(0,2,1),h_aten_Q,h_aten_Q[:,0:1])

        attention_data = torch.bmm(temp_qk_a,h_t_1)
        ...
        h_aten_obs_Q = F.relu(self.atten_obs_Q(state_features_obs))
        temp_qk_o = self.graph_softmax(attention_weight_obs.permute(0,2,1),h_aten_obs_Q,h_aten_Q[:,0:1])
        attention_data_obs = torch.bmm(temp_qk_o,h_t_obs_1)
