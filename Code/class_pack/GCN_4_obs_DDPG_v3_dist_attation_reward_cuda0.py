...
class GCN_Actor(nn.Module):
    def __init__(self, input_dim=4,sub_agent_num=4, obs_num=6):
        ...
        self.mean_out3 = nn.Linear(sub_agent_num*sub_agent_num*2,2)
        self.std_out3 = nn.Linear(sub_agent_num*sub_agent_num*2,2)
    ...
    def forward(self,attention_weight,state_features,attention_weight_obs,state_features_obs):
        ...
        out_mean = torch.tanh(self.mean_out3(h_out_2))
        out_std_log = self.std_out3(h_out_2)
        out_std_log = torch.clamp(out_std_log, -10, 2)
        out_std = out_std_log.exp()
