...
writer = SummaryWriter('train_loss_car/attention_v2Hz_len_4_train2') #loss_11 10, loss_10 005, loss_120 020
...
def train_data_At_construct(data, time_len):
    ...
    last_vel_time_data = np.zeros((raw_data_len - time_len +1,time_len,2))
    des_vel_data = data[time_len-1:raw_data_len,4:6]
    acture_vel_data = data[time_len-1:raw_data_len,0:2]
    for k in range(raw_data_len-time_len+1):
        last_vel_time_data[k] = data[k:k+time_len,2:4]
    return last_vel_time_data, des_vel_data , acture_vel_data 

time_len = 4
motion_model = RNN.RNN_net_AT_2(2,2,2,time_len).to(device)
...
if __name__ == '__main__':
    ...
    train_data = (np.load('car_data_6-108000-8.npy')/1.5)
    train_data0 = np.zeros((train_data.shape[1]*4,6))
    train_data0[0:train_data.shape[1]] = train_data[0,:,2:]#(np.loadtxt(open(file_name0),delimiter=",",skiprows=0)/1.5)[10:,:]
    train_data0[train_data.shape[1]:train_data.shape[1]*2] = train_data[1,:,2:]#(np.loadtxt(open(file_name1),delimiter=",",skiprows=0)/1.5)[10:,:]
    train_data0[train_data.shape[1]*2:train_data.shape[1]*3] = train_data[2,:,2:]
    train_data0[train_data.shape[1]*3:train_data.shape[1]*4] = train_data[3,:,2:]
    
    last_vel_all, des_vel_all, acture_vel_all = train_data_At_construct(train_data0,time_len)



    last_vel = last_vel_all[0:60000,:]
    des_vel = des_vel_all[0:60000,:]
    acture_vel = acture_vel_all[0:60000,:]

    val_last_vel = last_vel_all[60000:,:]
    val_des_vel = des_vel_all[60000:,:]
    val_acture_vel = acture_vel_all[60000:,:]
    ...
    for episode_i in range(episode_num):
        ...
        with torch.no_grad():
            ...
            if math.fabs(test_all_loss*1.0/test_count) < math.fabs(best_test_loss):
                ...
                mode_name='train_motion_model_best_car_attention_2Hz_len4_train2.pth'
                torch.save(motion_model,mode_name)
            if episode_i%100 ==0:
                mode_name='train_motion_model_car_412_attention_2Hz_len4_train2'+str(episode_i)+'.pth'
                torch.save(motion_model,mode_name)
